...
REVIEW_ROOT = REPO_ROOT / 'automation' / 'agents' / 'review'
...
def github_request(url: str, token: str) -> Any:
    request = urllib.request.Request(
        ...
        headers={
            ...
            'User-Agent': 'textbook-agent-ai-review',
        },
    )
...
def github_post(url: str, token: str, payload: dict[str, Any]) -> None:
    request = urllib.request.Request(
        url,
        headers={
            'Accept': 'application/vnd.github+json',
...
def collect_pr_files(repository: str, pr_number: int, token: str) -> list[dict[str, Any]]:
...
def build_review_payload(event: dict[str, Any], files: list[dict[str, Any]]) -> str:
    review_schema = load_text(REVIEW_ROOT / 'review-report.schema.json')
    bug_prompt = load_text(REVIEW_ROOT / 'bug-risk.md')
    architecture_prompt = load_text(REVIEW_ROOT / 'architecture-compliance.md')
    test_gap_prompt = load_text(REVIEW_ROOT / 'test-gap.md')
    ...
    prompt = f"""
You are an external code review agent for the Textbook Agent repository.
...
- Focus on bug risk, architectural violations, and missing or weak validation.
- Prefer fewer high-signal findings over speculative noise.
- Use severity values critical, high, medium, or low.
- Use category values bug-risk, architecture-compliance, or test-gap.
...
def post_pr_comment(repository: str, pr_number: int, token: str, body: str) -> None:
    url = f'https://api.github.com/repos/{repository}/issues/{pr_number}/comments'
    github_post(url=url, token=token, payload={'body': body})
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        files = collect_pr_files(repository=args.repository, pr_number=pr_number, token=token)
        prompt = build_review_payload(event=event, files=files)
        ...
        print(str(exc))
        return 0
    except (ReviewError, urllib.error.HTTPError, urllib.error.URLError, OSError, KeyError) as exc:
