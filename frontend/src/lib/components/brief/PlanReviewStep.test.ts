...
describe('PlanReviewStep', () => {
	...
	it('renders Generate as pack button when onGenerateAsPack is provided', async () => {
		const onGenerateAsPack = vi.fn();
		const onCommit = vi.fn();
		const onBack = vi.fn();
		...
		render(PlanReviewStep, {
			plan: {
				...
				lesson_rationale: 'Pack-ready plan.',
				directives: {
					tone_profile: 'supportive',
					reading_level: 'standard',
					explanation_style: 'balanced',
		...
		const packButton = screen.getByRole('button', { name: /generate as pack/i });
		expect(packButton).toBeTruthy();
		await fireEvent.click(packButton);
		expect(onGenerateAsPack).toHaveBeenCalledTimes(1);
		expect(onCommit).not.toHaveBeenCalled();
	});

	it('does not render Generate as pack button when onGenerateAsPack is absent', () => {
		render(PlanReviewStep, {
			plan: {
				id: 'plan-4',
				template_id: 'guided-concept-path',
				...
				lesson_rationale: 'No pack action.',
				directives: {
					tone_profile: 'supportive',
					reading_level: 'standard',
					explanation_style: 'balanced',
		...
		expect(screen.queryByRole('button', { name: /generate as pack/i })).toBeNull();
	});

	it('shows packError below actions when set', () => {
		render(PlanReviewStep, {
			plan: {
				id: 'plan-5',
				template_id: 'guided-concept-path',
				...
				lesson_rationale: 'Error visibility test.',
				directives: {
					tone_profile: 'supportive',
					reading_level: 'standard',
					explanation_style: 'balanced',
			...
			onBack: vi.fn(),
			onCommit: vi.fn(),
			onGenerateAsPack: vi.fn(),
			packError: 'Pack planning failed.'
		});

		expect(screen.getByText('Pack planning failed.')).toBeTruthy();
	});
});
