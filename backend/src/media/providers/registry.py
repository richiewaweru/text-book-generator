...
def load_image_provider_spec() -> ImageProviderSpec:
    ...
    if provider == "xai":
        if not base_url:
            raise RuntimeError(
                "IMAGE_BASE_URL is required when IMAGE_PROVIDER=xai"
            )
        if not api_key_env:
            raise RuntimeError(
                "IMAGE_API_KEY_ENV is required when IMAGE_PROVIDER=xai"
            )
        return ImageProviderSpec(
            provider="xai",
    ...
    raise RuntimeError("Unsupported IMAGE_PROVIDER. Expected one of: gemini, openai, xai.")
