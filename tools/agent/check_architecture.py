...
def extract_internal_imports(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if resolved and (resolved == module_prefix or resolved.startswith(f'{module_prefix}.')):
...
def find_violations(
    ...
    if strategy_type != 'import-layer':
        raise ContextError(
            'find_violations only supports import-layer strategies. Use the CLI for other strategy types.'
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if strategy_type == 'import-layer':
        violations = import_layer_violations(strategy, repo_root=repo_root)
